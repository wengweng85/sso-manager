...
public class WebRoleServicelmpl extends MvcHelper  implements WebRoleService {
	...
	public AjaxReturnMsg saveOrUpdateRoleData(SRole srole) {
		...
		if(ispermsionexist!=null){
			   return this.error("此角色"+srole.getRolecode()+"编号已经存在,请重新输入一个新的角色编号");
		}else{
			//判断是否更新操作
	...
	@Override
	public SRole getRoleByRoleCode(String rolecode) {
		return webRoleMapper.getRoleByRoleCode(rolecode);
	}
}
