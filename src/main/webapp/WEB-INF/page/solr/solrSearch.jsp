                        ...
                        <small>搜索用时  (<span id="qttime"></span>秒)</small>
                           {{#each this}}
	                        ...
	                        <div class="search-result">
	                            <h3><a target="_blank" href="{{{this.link_url}}}"> {{{this.title}}}</a></h3>
	                            <a target="_blank" href="{{{this.link_url}}}" class="search-link">{{{this.link_url}}}</a>
                                <div  width='100%' height='150' valign="top">                                  
                                    {{#if img}}
                                    <div style='width:200px;height:150px;' >
	                                   <img src='{{{this.img}}}' width='200' height='150'></img>
                                    </div>
                                    {{/if}}
                                    <div valign="top">
                                    {{{this.content}}}
	                        ...
	                        {{/each}}
			                    ...
			                    <input type="hidden" id="rows" value="100">
   ...
   $("#search").click(function() {
            ...
            var url= "<c:url value='/common/solrquery'/>?q="+rc.encodeURITwice(keyword)+"&start="+start+"&rows="+rows;
   			rc.ajax(url, null,function (response) {
   				...
   				if(response.success&&response.obj.list.length>0){
   					var model=$('#model').html();
   					var modeldata=Handlebars.compile($('#model').html());
   					var views = modeldata(response.obj.list);
   					$('#view').html(views);
