...
<div class="ibox ">
	<div class="ibox-title">
		...
		<div class="ibox-tools">
            ...
            <button onclick="s_perm_bat_delete()" class="btn btn-danger btn-xs"><i class="fa fa-remove"></i>批量删除</button>
        </div>
	</div>
...
<div class="col-sm-9">
	<div class="ibox ">
		<div class="ibox-title">
			<h5>权限编辑区</h5>
			<div class="ibox-tools">
			    <button  onclick="s_perm_savePermData()" class="btn btn-primary btn-xs"><i class="fa fa-save"></i>保存</button>
            </div>
		</div>
		<div class="ibox-content">
		<form action="<c:url value='/web/perm/saveorupdate'/>"  class="form-horizontal" method="post" id="myform">
           ...
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="permcode"  required="true"  cols="2,10" label="权限编码 " validate="{required:true,messages:{required:'请输入权限编码'}}"/>
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="permname"  required="true"  cols="2,10" label="权限名称" validate="{required:true,messages:{required:'请输入权限名称'}}"/>
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
               <rc:select codetype="PERMTYPE"  property="type"  label="权限类型"   dynamic="true"   cols="2,10" validate="{required:true,messages:{required:'请选择权限类型'}}"/> 
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
           ...
           <div class="hr-line-dashed"></div>
           <div class="form-group">
                <rc:textedit property="permdescribe" cols="2,10" label="权限描述" />
           </div>
           <div class="hr-line-dashed"></div>
           <div class="form-group">
...
function s_perm_addnewperm(){
	...
	var permname=$('#permname').val()||'权限头结点';
	rc.clean();
	$('#parentid').val(id);
	$('#parentname').val(permname);
	$('#url').val('#');
}

function sys_right_menu_perm_addnewperm(id,permname,systypecode){
	rc.clean();
	$('#parentid').val(id);
	$('#parentname').val(permname);
	$('#url').val('#');
...
function addDetailSubTreeNode() {
	...
	current_detail_selectedNodes=zDetailTree.getSelectedNodes()[0];
	sys_right_menu_perm_addnewperm(current_detail_selectedNodes.id,current_detail_selectedNodes.permname,current_detail_selectedNodes.systypecode);
}
